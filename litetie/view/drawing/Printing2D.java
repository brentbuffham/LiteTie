...
public class Printing2D {
	...
	final static BasicStroke LINE_015 = new BasicStroke(0.15f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER);
...
//			LiteTieTRIAL.setConsoleOutput("\n"+default_FONT.getName() +" size:"+default_FONT.getSize());
	...
	public Color getInverseColour(Color colour){
		int max = 255;
		int currentRed = colour.getRed();
		int currentGreen = colour.getGreen();
		int currentBlue = colour.getBlue();
		colour = new Color(max-currentRed, max-currentGreen, max-currentBlue);
		return colour;	
	}
	...
	public void paintHole(Graphics2D g2, Hole hole, double lastEasting, double lastNorthing, double mouseE, double mouseN, Rectangle2D bounds, double scale, double enhance,boolean selected, boolean moveToolSelected, int shape, double enhanceTie) {
		...
		double centreX = ((hole.getX() - bounds.getX()) * scale);
		double centreY =  ((bounds.getHeight() + bounds.getY() - hole.getY()) * scale);
		...
		double lowX = (((hole.getX() - bounds.getX()) + (UnitConvert.pixelsToMeters(hole.getDiameter()*enhance)) / 2) * scale);
		double lowY = (((bounds.getHeight() + bounds.getY() - hole.getY()) + (UnitConvert.pixelsToMeters(hole.getDiameter()*enhance)) / 2) * scale);
		double centreXunScaled = hole.getX()-bounds.getX();
		double centreYunScaled = (bounds.getHeight() + bounds.getY() - hole.getY());
		//CIRCLE
		if(shape == 1){
		...
		//TRIANGLE
		if(shape == 3){
			double [] xs = {(((hole.getX() - bounds.getX()) * scale)), 	(((hole.getX() - bounds.getX()) - (UnitConvert.pixelsToMeters(hole.getDiameter()*enhance)) / 2) * scale),(((hole.getX() - bounds.getX()) + (UnitConvert.pixelsToMeters(hole.getDiameter()*enhance)) / 2) * scale)};
			double [] ys = {(((bounds.getHeight() + bounds.getY() - hole.getY()) - (UnitConvert.pixelsToMeters(hole.getDiameter()*enhance)) / 2) * scale), (((bounds.getHeight() + bounds.getY() - hole.getY()) + (UnitConvert.pixelsToMeters(hole.getDiameter()*enhance)) / 2) * scale),(((bounds.getHeight() + bounds.getY() - hole.getY()) + (UnitConvert.pixelsToMeters(hole.getDiameter()*enhance)) / 2) * scale)};
			int n = 3;
			g2.setStroke(LINE_025);
			g2.setColor(hole.getColor());
		...
		if(shape == 4){
			g2.setStroke(LINE_025);
			g2.setColor(hole.getColor());
			g2.draw(new Rectangle2D.Double(hx,hy,hw,hh));
		}
		...
		if(shape == 5){
			...
			for(int i=0; i<n; i++) {
				...
				x[i] = ((centreXunScaled + r*Math.cos(v))*scale);
				y[i] = ((centreYunScaled - r*Math.sin(v))*scale);
			}
			g2.setStroke(LINE_025);
			g2.setColor(hole.getColor());
		...
		if(shape == 7){
			...
			for(int i=0; i<n; i++) {
				if(i%2 == 0){
					double v = (i*angle) - Math.toRadians(turn);
					x[i] =  ((centreXunScaled + (r/4)*Math.cos(v))*scale);
					y[i] =  ((centreYunScaled - (r/4)*Math.sin(v))*scale);
				}
				else{
					double v = (i*angle) - Math.toRadians(turn);
					x[i] =  ((centreXunScaled + r*Math.cos(v))*scale);
...
//		g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
		...
		g2.fill(new Ellipse2D.Double(
				((((hole.getX() - bounds.getX()) - (UnitConvert.pixelsToMeters(hole.getDiameter()*enhance / 2))) + toeLoc	* Math.cos(Math.toRadians(90 - hole.getBearing()))) * scale),
				(((bounds.getHeight() + bounds.getY()	- hole.getY() - (UnitConvert.pixelsToMeters(hole.getDiameter()*enhance / 2))) - ((toeLoc * Math.sin(Math.toRadians(90 - hole.getBearing()))))) * scale),
				(UnitConvert.pixelsToMeters(hole.getDiameter()*enhance) * scale),
				(UnitConvert.pixelsToMeters(hole.getDiameter()*enhance) * scale)));
		//toeCircle
		g2.setColor(p_toeLocationLineColour);
...
//		g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
		...
		if (!(selected)) {g2.setColor(Color.GRAY);	g2.setStroke(DASH_050_008);	}
		else {	g2.setColor(p_selectedObjectsColour); g2.setStroke(LINE_075);		}
		if (!(selected)) {	g2.setColor(Color.GRAY);	g2.setStroke(DASH_050_002);	}
		else {	g2.setColor(p_selectedObjectsColour);	g2.setStroke(LINE_075);		}
		//			floor RL marker as a dashed Circle.
		g2.setColor(p_floorLocationFillColour);
		g2.fill(new Ellipse2D.Double(
				((((hole.getX() - bounds.getX()) - (UnitConvert.pixelsToMeters(hole.getDiameter()*enhance/2))) + floorLoc*Math.cos(Math.toRadians(90-hole.getBearing())))*scale),
				(((bounds.getHeight() + bounds.getY() - hole.getY()- (UnitConvert.pixelsToMeters(hole.getDiameter()*enhance/2))) - ((floorLoc*Math.sin(Math.toRadians(90-hole.getBearing())))))*scale),
				(UnitConvert.pixelsToMeters(hole.getDiameter()*enhance) *scale ),
				(UnitConvert.pixelsToMeters(hole.getDiameter()*enhance)* scale)));
		g2.setColor(p_floorLocationLineColour);
		g2.draw(new Ellipse2D.Double(
...
*/
	...
	public void paintMarquee(Graphics2D g2, Rectangle2D bounds, Point2D start, Point2D end, double scale, String selectionType, boolean selectRect, boolean selectElipse) {
		...
		g2.setColor(new Color(p_selectedObjectsColour.getRed(),p_selectedObjectsColour.getGreen(),p_selectedObjectsColour.getBlue(),30));//transparent Color
...
//		g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
		...
		for (int i = 0; i < x.size(); i++){
			...
			g2.setFont(new Font("Dialog", Font.BOLD,18 ));
			g2.drawString(selectionType, (int)((x.get(0).doubleValue() - bounds.getX())*scale),(int)((bounds.getHeight() +bounds.getY() - y.get(0).doubleValue())*scale));
		}
		int nPoints = x.size();
...
//		g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
	...
	//RULER MARKERS THAT PRESENT PREPENDICULAR TO THE LINE	
		g2.setColor(DARK_CYAN);
		g2.drawLine((int)((start.getX()- bounds.getX())*scale), 
				(int)((bounds.getHeight() + bounds.getY()-start.getY())*scale), 
				(int)((end.getX()- bounds.getX())*scale), 
				(int)((bounds.getHeight() + bounds.getY()-end.getY())*scale));
		//METRE INCREMENTS
		for (int i = 0; i <= d ; i++) {
...
//		grid2D.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_OFF);
		...
		grid2D.setColor(p_gridLinesAndTextColour);
		{
			...
			for (double i =  (bounds.getHeight()); i > 0; i = i
					//meters Northing
					-  gridDisplay.getHeight()) {
				...
				grid2D.setFont(grid_FONT);
				grid2D.drawString(vertText, 0, (int) (i
						+ grid2D.getFontMetrics().getHeight()));
				//System.out.println("gridDraw vertical text");
				grid2D.setFont(grid_FONT);
				grid2D.drawString(vertText, ((int) bounds.getWidth())
						- grid2D.getFontMetrics().stringWidth(vertText), (int) (i
						+ grid2D.getFontMetrics().getHeight()));
